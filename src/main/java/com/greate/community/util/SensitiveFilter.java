...
public class SensitiveFilter {
    ...
    /**
     * 过滤敏感词
     * TrieNode 只有一个子节点和isKeywordEnd字段
     * 过滤时维护一个开始指针和结束指针分别指向敏感词的第一和最后一位，遍历传入text的字符，如果开始指针对应的字符命中敏感词，则将当前的Node
     * 指向该敏感词所在的Node，往后遍历字符的同时遍历Node，直到字符无法命中敏感词或完全命中敏感词。完全命中后做替换，同时将Node指回根节点。
     *
     * @param text 待过滤的文本
